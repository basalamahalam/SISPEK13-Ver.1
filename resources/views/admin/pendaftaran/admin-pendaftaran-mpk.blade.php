...
<div class="main-content">
    ...
    <main>
        <div>
            ...
            <!-- LINK -->
            <a href="{{ URL::to('/admin-pendaftaran-mpk') }}" class="dash-title">Pendaftaran</a>
            /
            <a href="{{ URL::to('/admin-pendaftaran-mpk') }}" class="link-title">MPK</a>
        </div>
            ...
            <table>
                ...
                <tbody>
                    @php
                        $i = 0;
                    @endphp
                    @if ($signMpk->isNotEmpty())
                        @foreach ($signMpk as $sign)
                            <tr>
                                <td>{{ ++$i }}</td>
                                ...
                                <td>{{ $sign->gender }}</td>
                                <td class="btn-table">
                                    <form action="/admin-pendaftaran-mpk/{{ $sign->id }}" method="POST">
                                        <a href="/admin-pendaftaran-mpk-detail/{{ $sign->id }}/detail-/{{ $sign->pendaftaran }}/{{ $sign->nama }}"
